...
function calc() {
  ...
  let quality, qClass;
  if      (!validEntry)            { quality = '—';                  qClass = 'cm'; }
  else if (gapTier === 'merged')   { quality = 'D';                   qClass = 'cr'; }
  else if (gapTier === 'weak')     { quality = 'C';                   qClass = 'ca'; }
  else if (gapTier === 'moderate') { quality = timeOk ? 'B+' : 'B';   qClass = 'ca'; }
  else                              { quality = timeOk ? 'A+' : 'A';   qClass = isBull?'cg':'cr'; }
  ...
  document.getElementById('metrics').innerHTML = `
    ...
    <div class="metric"><div class="metric-label">Quality</div><div class="metric-value ${qClass}">${quality}</div><div class="metric-sub">${gapTierInfo.short}${validEntry&&!timeOk?' · outside window':''}</div></div>`;
  ...
  if (validEntry) {
    ...
    // Further-OTM strikes are only offered when cloud separation (gapTier)
    // actually supports the move reaching that far — thin gaps stay ATM/1-OTM only.
    const rows = [
      { label: `Aggressive <span class="badge bm" style="font-size:10px;padding:2px 7px">ATM</span>`,    strike: atm,         entry: f(price) },
    ...
    if (gapTier === 'moderate' || gapTier === 'strong') {
    ...
    rows.push({ label: `${isBull?'Pullback':'Bounce'} <span class="badge ${isBull?'bg':'br'}" style="font-size:10px;padding:2px 7px">best price</span>`, strike: atm, entry: `${pullEntry} (${pullLbl})` });
    ...
    document.getElementById('etbody').innerHTML = rows.map(r => `
      ...
      (gapTier === 'weak' || gapTier === 'merged'
        ? `<tr><td colspan="6"><div class="wait-box" style="background:var(--bg3);border-color:var(--border2);color:var(--muted)"><span style="font-size:16px;flex-shrink:0">ℹ</span><div>Gap too thin for further OTM strikes — stick to ATM/1 OTM until cloud separation widens.</div></div></td></tr>`
        : '');
